...
#==============================================================================
N = 2**18 # Datalængde
freq = 1000 # Samplingsfrekvens i Hz
...
def sig(x):
    if x <= t/14.:
        return np.sin(15*x**2)
    if t/14. < x <= 2*t/14.:
        return np.sin(15*x**2)+ np.sin(400*x)
    if 2*t/14. < x <= 3*t/14.:
        return np.sin(15*(x-4*t/14.)**2) + np.sin(400*x)
    if 3*t/14. < x <= 4*t/14.:
        return np.sin(15*(x-4*t/14.)**2) 
    
    if 4*t/14. < x <= 5*t/14.:
        return np.sin(x)
    
    if x <= 6*t/14.:
        return np.sin(15*(x-5*t/14.)**2)
    if 6*t/14. < x <= 7*t/14.:
        return np.sin(15*(x-5*t/14.)**2)+ np.sin(400*x)
    if 7*t/14. < x <= 8*t/14.:
        return np.sin(15*(x-9*t/14.)**2) + np.sin(400*x)
    if 8*t/14. < x <= 9*t/14.:
        return np.sin(15*(x-9*t/14.)**2) 
    
    if 9*t/14. < x <= 10*t/14.:
        return np.sin(x)

    if 10*t/14. < x <= t:
        return np.sin(0.005*(x-12*t/14.)**4)
...
bins = 2*np.pi*np.linspace(0,freq/2.,N/2.) # Frekvensintervaller
...
vmin = np.min(X)
vmax = np.max(X)
spec = plt.pcolormesh(x,y[freq_inter1:freq_inter2],X[freq_inter1:freq_inter2],cmap='jet',vmin=-130,vmax=vmax)
