...
void highprio()
{
  while (1) {
    ...
    k_eat_time(200);  // lets eat 200 milliseconds
    k_wait(sem1, 0);
...
extern "C" {
  ...
  {
    unsigned char c;
    PORTB  = (PORTB & 0x0F) |  (1 << (pRun->nr + 4)); // arduino uno !! specific usage of PORTB
    PORTC = (PORTC & 0xef)| led13;
  }

  void k_sem_clip(unsigned char nr, int i)
  {
    led13 = 0x40; // LED13 is bit 5 so sem overflow will in fct below switch on LED13
